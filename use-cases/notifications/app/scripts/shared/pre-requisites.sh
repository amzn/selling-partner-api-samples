...
echo "To deploy this Sample Solution App, the app-config.json has to be updated to match your desired configuration."
while true; do
  read -p "Have you updated 'app-config.json' with values matching what you are aiming to deploy? [y/n] " response
  case "${response}" in
    [yY][eE][sS]|[yY]) break;;
    [nN][oO]|[nN])
      echo "Please update 'app-config.json', and execute this script afterwards."
      exit -1;;
    *) echo "Please answer 'yes' or 'no'";;
  esac
done
...
if check_nodejs; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        ...
        echo "Unsupported operating system. The script supports macOS and Windows only. Install NodeJS manually before restarting."
        exit 1
    fi
fi
