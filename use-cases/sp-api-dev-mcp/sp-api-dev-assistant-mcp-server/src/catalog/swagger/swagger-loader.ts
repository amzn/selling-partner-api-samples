...
export class SwaggerLoader {
   ...
   */
  async loadSwaggerFiles(
    directoryPath: string,
  ): Promise<Map<string, SwaggerDocument>> {
    ...
    try {
      ...
      const swaggerFiles =
        await this.findSwaggerFilesRecursively(directoryPath);

      logger.info(
        `Found ${swaggerFiles.length} potential Swagger/OpenAPI files`,
      );
      ...
      for (const filePath of swaggerFiles) {
        try {
          ...
          if (this.isValidSwaggerDocument(swagger)) {
            ...
            logger.warn(
              `File does not appear to be a valid Swagger/OpenAPI document: ${filePath}`,
            );
          }
        } catch (error) {
      ...
      logger.info(
        `Successfully loaded ${swaggerMap.size} Swagger/OpenAPI documents`,
      );
      return swaggerMap;
    } catch (error) {
   ...
   */
  private async findSwaggerFilesRecursively(
    directoryPath: string,
  ): Promise<string[]> {
  ...
  processSwagger(swagger: SwaggerDocument): ProcessedSwaggerDocument {
    logger.debug(
      `Processing Swagger document: ${swagger.info?.title || "Unnamed API"}`,
    );
    ...
    try {
      ...
      ) as ProcessedSwaggerDocument;

      // Resolve references with circular detection
      processed = this.processReferencesWithCircularDetection(
        processed,
        processed,
        new Set(),
        [],
  ...
  private processReferencesWithCircularDetection(
    ...
    if (Array.isArray(current)) {
      return current.map((item, index) =>
        this.processReferencesWithCircularDetection(swagger, item, visited, [
          ...path,
          `[${index}]`,
        ]),
      );
    ...
    if (current.$ref && typeof current.$ref === "string") {
      ...
      if (visited.has(ref)) {
        logger.warn(
          `Circular reference detected: ${ref} in path ${currentPath}`,
        );
        return {
          type: "object",
