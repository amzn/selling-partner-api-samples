...
export class SwaggerReferenceResolver {
  ...
  resolveReferences(obj: any, context: string[] = [], depth: number = 0): any {
    ...
    if (depth > 100) {
      logger.warn(
        `Maximum reference depth exceeded at context: ${context.join("/")}`,
      );
      return {
        type: "object",
        description: "Maximum reference depth exceeded",
      };
    }
    ...
    if (Array.isArray(obj)) {
      return obj.map((item) =>
        this.resolveReferences(item, context, depth + 1),
      );
    }
    ...
    if (obj.type === "array" && obj.items) {
      resolved.items = this.resolveReferences(
        obj.items,
        [...context, "items"],
        depth + 1,
      );
  ...
  private handleSchemaCombiner(
    ...
    for (const combiner of ["allOf", "anyOf", "oneOf"]) {
      if (resolved[combiner] && Array.isArray(resolved[combiner])) {
        resolved[combiner] = resolved[combiner].map(
   ...
   */
  private resolveCommonSchemaProperties(
    resolved: any,
    context: string[],
    depth: number,
  ): void {
      ...
      if (ref.startsWith("#/")) {
        ...
        for (const segment of path) {
          ...
          if (!target[decodedSegment]) {
            logger.warn(
              `Invalid reference: ${ref} at context: ${context.join("/")}`,
            );
