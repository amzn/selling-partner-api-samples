...
export class CatalogMapper {
   ...
   */
  mapSwaggerToCatalog(
    swaggerMap: Map<string, ProcessedSwaggerDocument>,
  ): ApiCatalog {
    ...
    logger.info(
      `API catalog mapping complete with ${catalog.categories.length} categories`,
    );
    return catalog;
  }
  ...
  private generatePurpose(operation: Operation): string {
    ...
    switch (method) {
      case "GET":
        return "Retrieve information from Amazon SP-API.";
      case "POST":
        return "Create or submit data to Amazon SP-API.";
      case "PUT":
        return "Update existing data in Amazon SP-API.";
      case "DELETE":
        return "Remove data from Amazon SP-API.";
      default:
        return "Interact with Amazon SP-API.";
    }
  }
    ...
    if (useCases.length === 0) {
      ...
      switch (method) {
        case "GET":
          ...
          if (
            operation.operationId?.includes("list") ||
            operation.operationId?.includes("search")
          ) {
            useCases.push(
              `Find specific ${resource} that match certain criteria.`,
            );
          }
          break;
        case "POST":
          ...
          if (
            operation.operationId?.includes("submit") ||
            operation.operationId?.includes("send")
          ) {
          ...
          for (const [headerName, headerSpec] of Object.entries(
            ...
            if (headerSpec && typeof headerSpec === "object") {
              ...
              if ("type" in headerSpec && "description" in headerSpec) {
                ...
                "schema" in headerSpec &&
                headerSpec.schema &&
                typeof headerSpec.schema === "object" &&
                "type" in headerSpec.schema
              ) {
  ...
  private findRelatedEndpoints(
    ...
    for (const [path, pathItem] of Object.entries(swagger.paths)) {
      for (const method of ["get", "post", "put", "delete"]) {
        ...
        const sameResource =
          resourcePath && opResourcePath && resourcePath === opResourcePath;
  ...
  private generateIntentMappings(categories: ApiCategory[]): IntentMapping[] {
    ...
    for (const category of categories) {
      for (const endpoint of category.endpoints) {
        ...
        const primaryIntent = this.generatePrimaryIntent(
          endpoint,
          category.name,
        );
        intentMappings.push({
        ...
        for (const useCase of endpoint.commonUseCases) {
          ...
          if (
            useCaseIntent &&
            !this.intentExists(intentMappings, useCaseIntent)
          ) {
            intentMappings.push({
        ...
        this.addOperationSpecificIntents(
          endpoint,
          category.name,
          intentMappings,
        );
   ...
   */
  private intentExists(
    intentMappings: IntentMapping[],
    intent: string,
  ): boolean {
    return intentMappings.some(
      (mapping) => mapping.intent.toLowerCase() === intent.toLowerCase(),
    );
  }
}
