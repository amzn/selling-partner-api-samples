...
export class CatalogLoader {
  ...
  async loadCatalog(): Promise<ApiCatalog> {
    ...
    try {
      ...
      this.catalog =
        this.catalogMapper.mapSwaggerToCatalog(processedSwaggerMap);

      logger.info(
        `API catalog loaded successfully: ${this.catalog.categories.length} categories with ${this.getTotalEndpoints(this.catalog)} total endpoints`,
      );
      return this.catalog;
    } catch (error) {
