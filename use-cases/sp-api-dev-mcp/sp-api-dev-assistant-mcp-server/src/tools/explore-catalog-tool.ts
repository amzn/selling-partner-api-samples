        ...
        if (val < 0) {
          ctx.addIssue({
            ...
            message: JSON.stringify({
              ...
              validExamples: [{ depth: 0 }, { depth: 1 }, { depth: 2 }],
            }),
          });
          return z.NEVER;
        }
...
export const exploreCatalogSchema = z.object({
  endpoint: z
    .string()
    .optional()
    .describe("Specific endpoint to get details for"),
  category: z.string().optional().describe("Category to explore"),
  listEndpoints: z
    .boolean()
    .optional()
    .default(false)
    .describe("List all available endpoints"),
  listCategories: z
    .boolean()
    .optional()
    .default(false)
    .describe("List all available categories"),
  depth: createDepthValidator(),
  ref: z
    .string()
    .optional()
    .describe(
      "Extract specific nested object using dot notation (e.g., 'Order.ShippingAddress')",
    ),
});
...
export class ExploreCatalogTool {
      ...
      return JSON.stringify(
        {
          ...
          metadata: {
            ...
            suggestedActions: this.generateActionableSuggestions(
              params,
              truncatedFields,
            ),
          },
          suggestions: {
            progressive_exploration: [
              currentDepth === "full"
                ...
                : `Try depth: ${Math.max(0, Number(currentDepth) - 1)} for less detail`,
              currentDepth === "full"
                ? "Try depth: 2 for moderate detail"
                : `Try depth: ${Number(currentDepth) + 1} for more detail`,
  ...
  private generateActionableSuggestions(
    ...
    if (truncatedFields.length > 0) {
      suggestions.push(
        `Found ${truncatedFields.length} truncated field(s) - try increasing depth`,
      );
    }

    if (typeof currentDepth === "number") {
      suggestions.push(
        `Current depth: ${currentDepth}, try depth: ${currentDepth + 2} or higher for more detail`,
      );
      suggestions.push(`Try depth: 'full' for complete expansion`);
    } else if (currentDepth === "full") {
      suggestions.push(
        "Using 'full' depth - response may be truncated due to size limits",
      );
    ...
    if (params.endpoint && !params.ref) {
      suggestions.push(
        "Use 'ref' parameter to explore specific nested objects (e.g., 'properties.payload')",
      );
    }
  ...
  private extractReference(
    ...
    const successResponse =
      endpoint.responses.find((r) => r.statusCode === 200) ||
    ...
    const extractedSchema = this.navigateSchemaPath(
      ...
      return JSON.stringify(
        {
          ...
          suggestions: this.generatePathSuggestions(
            successResponse.schema,
            refPath,
          ),
          navigation_context: this.analyzeCurrentLocation(
            successResponse.schema,
            "",
          ),
        },
    ...
    const limitedSchema = this.limitSchemaDepth(
      extractedSchema,
      params.depth || "full",
    );
    return JSON.stringify(
      {
        ...
        metadata: {
          ...
          schema_truncated:
            JSON.stringify(limitedSchema).includes("[Object:") ||
            JSON.stringify(limitedSchema).includes("[Array]"),
          suggestions: this.generateActionableSuggestions(params, []),
        },
      },
      null,
  ...
  private analyzeCurrentLocation(schema: any, currentPath: string): object {
    ...
    if (schema.properties) {
      ...
      context.available_next_steps = fields.map(
        (field) => `${currentPath ? currentPath + "." : ""}properties.${field}`,
      );
    }

    if (schema.items) {
      context.available_next_steps.push(
  ...
  private generatePathSuggestions(
    ...
    if (schema && typeof schema === "object") {
      ...
      const exactMatches = availableKeys.filter(
        (key) => key.toLowerCase() === firstPart.toLowerCase(),
      );

      // Partial matches at top level
      const partialMatches = availableKeys.filter(
        (key) =>
      ...
      suggestions.push(
        ...partialMatches.filter((m) => !exactMatches.includes(m)),
      );
    ...
    if (typeof schema === "object" && schema !== null) {
      if (Array.isArray(schema)) {
        return schema.map((item) =>
          this.limitSchemaDepth(item, maxDepth, currentDepth + 1),
        );
      } else {
        const result: any = {};
        for (const [key, value] of Object.entries(schema)) {
          result[key] = this.limitSchemaDepth(
            value,
            maxDepth,
            currentDepth + 1,
          );
    ...
    if (endpoint.parameters.length > 0) {
      ...
      if (pathParams.length > 0) {
        ...
        for (const param of pathParams) {
          const typeInfo = Array.isArray(param.type)
            ? param.type.join(" | ")
            : param.type;
