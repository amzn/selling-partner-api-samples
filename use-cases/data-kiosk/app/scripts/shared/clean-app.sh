...
datakiosk_schedule_group_name="$(grep "^datakiosk_schedule_group_name=" "${resources_file_path}" | cut -d"=" -f2)"
datakiosk_bucket_name="$(grep "^datakiosk_bucket_name=" "${resources_file_path}" | cut -d"=" -f2)"
...
aws s3 rm s3://${datakiosk_bucket_name} --recursive

#Delete Ongoing Schedules
aws scheduler delete-schedule-group --name "${datakiosk_schedule_group_name}"
echo "Deleted Event Bridge Schedules stack"
