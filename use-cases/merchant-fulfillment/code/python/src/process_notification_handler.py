...
def lambda_handler(event, context):
    logger.info("ProcessNotification Lambda started")
    ...
    for message in event[constants.NOTIFICATION_MESSAGE_KEY_NAME]:
        ...
        try:
            ...
            if order_summary[constants.NOTIFICATION_FFM_TYPE_KEY_NAME] == constants.MFN_FULFILLMENT_CODE and order_summary[constants.NOTIFICATION_ORDER_STATUS_KEY_NAME] in constants.NOTIFICATION_PROCESSING_STATUS_ALL:
...
def start_step_functions_execution(order_notification):
    request = {
        ...
        'input': json.dumps({
            ...
            constants.STATE_MACHINE_REFRESH_TOKEN_KEY_NAME: os.environ.get(constants.REFRESH_TOKEN_ARN_ENV_VARIABLE),
            constants.STATE_MACHINE_REGION_CODE_KEY_NAME: os.environ.get(constants.REGION_CODE_ARN_ENV_VARIABLE)
        })
    }
