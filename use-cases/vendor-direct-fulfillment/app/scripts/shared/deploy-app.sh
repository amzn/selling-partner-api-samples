...
if [ "$language" == "java" ]; then
  ...
  lambda_func_handler="lambda.DummyLambdaHandler"
...
# If it's a Python app, package the code and upload it to S3
  ...
  get_dfOrders_handler="spApiCsharpApp::spApiCsharpApp.getVendorDFOrders::getDFOrdersHandler"
  acknowledge_dfOrder_handler="spApiCsharpApp::spApiCsharpApp.acknowledgeDFOrders::acknowledgeDFOrdersHandler"
  get_shippingLabel_handler="spApiCsharpApp::spApiCsharpApp.getShippingLabel::getShippingLabelHandler"
  check_transactionStatus_handler="spApiCsharpApp::spApiCsharpApp.checkTransactionStatus::checkTransactionStatusHandler"
  (
...
AWS_ACCESS_KEY_ID=${access_key} AWS_SECRET_ACCESS_KEY=${secret_key} \
  aws cloudformation create-stack \
    ...
    --parameters \
      ...
      ParameterKey="GetDFOrdersLambdaFunctionHandler",ParameterValue="${get_dfOrders_handler}" \
      ParameterKey="AcknowledgeDfOrderLambdaFunctionHandler",ParameterValue="${acknowledge_dfOrder_handler}" \
      ParameterKey="GetShippingLabelDfOrderLambdaFunctionHandler",ParameterValue="${get_shippingLabel_handler}" \
      ParameterKey="CheckTransactionStatusLambdaFunctionHandler",ParameterValue="${check_transactionStatus_handler}" \
