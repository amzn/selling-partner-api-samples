...
class LwaOAuthClient {
   ...
   */
  constructor(clientId, clientSecret, refreshToken, scopes) {
    if (refreshToken && scopes) {
      throw new Error(
        "Either refreshToken or scopes should be provided. But not both.",
      );
    } else if (!refreshToken && !scopes) {
      throw new Error("Neither refreshToken nor scopes provided.");
    }
   ...
   */
  #doRefresh = async () => {
    if (this.#lwaClientInfo.refreshToken) {
      ...
      const res = await superagent
        .post("https://api.amazon.com/auth/o2/token")
        .send(
          `grant_type=client_credentials&scope=${this.#lwaClientInfo.scopes.join(
            " ",
          )}&client_id=${this.#lwaClientInfo.clientId}&client_secret=${
            this.#lwaClientInfo.clientSecret
          }`,
...
export class ApiClient {
   ...
   * @returns {ApiClient} This ApiClient, which is going to use give accessToken for all API calls.
   */
  enableAutoRetrievalAccessTokenForScopes(clientId, clientSecret, scopes) {
    if (!clientId || !clientSecret || !scopes) {
      throw new Error(
        "invalid parameter(s) to enableAutoRetrievalAccessTokenForScopes.",
      );
    }
    this.#lwaClient = new LwaOAuthClient(
      clientId,
