 ...
 * Selecting the option with Non-Amazon Partnered Carrier (Your own transportation carrier).
 */
    ...
    public Map<String, Object> handleRequest(ListTransportationInput input, Context context) {
        ...
        try {
            ...
            if (!allTransportationOptions.isEmpty()) {
                // Search for the first transportation option with the USE_YOUR_OWN_CARRIER shipping solution and GROUND_SMALL_PARCEL shipping mode
                ...
                for (TransportationOption option : allTransportationOptions) {
                    if ("USE_YOUR_OWN_CARRIER".equals(option.getShippingSolution()) &&
                            "GROUND_SMALL_PARCEL".equals(option.getShippingMode())) {
            ...
            // Check if a Amazon Partnered Carrier transportation option is found
            if (selectedOption != null) {
                ...
                LambdaUtils.logMessage(context, "No transportation options found with USE_YOUR_OWN_CARRIER & GROUND_SMALL_PARCEL");
                response.put(TransportationResponseKey.ERROR.getKey(), "No transportation options found with USE_YOUR_OWN_CARRIER & GROUND_SMALL_PARCEL");
            }
