...
if [ "$language" == "java" ]; then
  ...
  generate_packing_options_func_handler="lambda.GeneratePackingOptionsLambdaHandler"
  list_packing_options_func_handler="lambda.ListPackingOptionsLambdaHandler"
  confirm_packing_option_func_handler="lambda.ConfirmPackingOptionLambdaHandler"
  ...
  list_inbound_plan_boxes_handler="lambda.ListInboundPlanBoxesLambdaHandler"
  update_shipment_tracking_details_handler="lambda.UpdateShipmentTrackingDetailsLambdaHandler"
  cancel_inbound_plan_func_handler="lambda.CancelInboundPlanLambdaHandler"
...
# Upload the Cancel Inbound Plan StepFunctions state machine definition to S3
state_machine_cancel_s3_key="step-functions/step-functions-cancelInboundPlan-workflow-definition.json"
AWS_ACCESS_KEY_ID=${access_key} AWS_SECRET_ACCESS_KEY=${secret_key} \
  aws s3 cp ../../step-functions/step-functions-cancelInboundPlan-workflow-definition.json "s3://${bucket_name}/${state_machine_cancel_s3_key}"
if [ $? -ne 0 ]
then
  echo "State machine definition upload to S3 failed"
  echo "Aborting"
...
AWS_ACCESS_KEY_ID=${access_key} AWS_SECRET_ACCESS_KEY=${secret_key} \
  aws cloudformation create-stack \
    ...
    --parameters \
      ...
      ParameterKey="SPAPIGeneratePackingOptionsLambdaFunctionHandler",ParameterValue="${generate_packing_options_func_handler}" \
      ParameterKey="SPAPIListPackingOptionsLambdaFunctionHandler",ParameterValue="${list_packing_options_func_handler}" \
      ParameterKey="SPAPIConfirmPackingOptionLambdaFunctionHandler",ParameterValue="${confirm_packing_option_func_handler}" \
      ...
      ParameterKey="SPAPIListInboundPlanBoxesLambdaFunctionHandler",ParameterValue="${list_inbound_plan_boxes_handler}" \
      ...
      ParameterKey="SPAPICancelInboundPlanLambdaFunctionHandler",ParameterValue="${cancel_inbound_plan_func_handler}" \
